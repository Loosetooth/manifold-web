...
export const examples = {
  functions: {
    ...
    TetrahedronPuzzle: function () {
      ...
      const result = tet
        .intersect(screw.rotate([0, 0, -45]).translate([0, 0, -1]))
        .scale(scale);
    ...
    TorusKnot: function () {
      ...
      const func = (v) => {
        ...
        vec3.set(
          v,
          threadRadius * Math.cos(phi),
          0,
          threadRadius * Math.sin(phi)
        );
      ...
      function fractal(holes, hole, w, position, depth, maxDepth) {
        ...
        for (let offset of offsets)
          fractal(
            ...
            hole,
            w,
            vec2.add(offset, position, offset),
            depth + 1,
            maxDepth
          );
      }
    ...
    StretchyBracelet: function () {
      ...
      function base(
        ...
        const result = Manifold.intersection(
          Manifold.extrude(stretch, width),
          b
        );
        return result;
      }
    ...
    GyroidModule: function () {
      ...
      function gyroidOffset(level) {
        ...
        return Manifold.levelSet(gyroid, box, period / n, level).scale(
          size / period
        );
      }
      ...
      const result = gyroidModule
        .rotate([-45, 0, 90])
        .translate([0, 0, size / Math.sqrt(2)]);
      return result;
    },
  },
